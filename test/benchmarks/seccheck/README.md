...
~/go/bin/benchstat redis_baseline.log redis_null_sink.log redis_remote_sink.log
```
...
trace, start a local web server (using `-no_browser` prevents X11 crashes on
headless machines):
...
```text
                       │ redis_baseline.log │        redis_null_sink.log        │       redis_remote_sink.log        │
...
operations throughput by **~4%**. This proves the Sentry's internal tracing
hooks are lean and create minimal GC churn.
